...
from test_transactions_page import BudgetRequestHandler, PageContract, ThreadingHTTPServer
...
class PeriodComparisonRouteTests(unittest.TestCase):
    ...
    def test_report_and_all_script_dependencies_do_not_create_a_database(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            thread.start()
            try:
                resources = ["/compare-periods"] + [
                    attrs.get("src") if tag == "script" else attrs.get("href")
                    for tag, attrs in PageContract("period-comparison.html").elements
