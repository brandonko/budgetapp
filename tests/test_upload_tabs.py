...
class UploadTabsTests(unittest.TestCase):
    ...
    def test_import_preview_requires_confirmation_and_supports_cancellation(self) -> None:
        ...
        self.assertIn('id="import-review-search"', html)
        self.assertIn('id="import-review-filter-popover"', html)
        self.assertIn('id="import-review-category-filter"', html)
        self.assertIn('id="import-review-subcategory-filter"', html)
        self.assertIn('id="import-review-tag-filter"', html)
        self.assertIn('id="import-review-account-filter"', html)
        self.assertIn('id="import-review-provider-filter"', html)
        review_dialog_start = html.index('id="import-review-dialog"')
        self.assertLess(
            html.index('class="transaction-toolbar"', review_dialog_start),
            html.index('class="import-review-filters"', review_dialog_start),
        )
        self.assertIn('data-review-filter="duplicate"', html)
        self.assertIn('data-review-filter="unmatched"', html)
        self.assertIn('data-review-filter="new"', html)
        self.assertIn('aria-pressed="false">Duplicate</button>', html)
        self.assertIn('aria-pressed="true">No rule matched</button>', html)
        self.assertIn('aria-pressed="true">New</button>', html)
        self.assertIn('state.reviewFilters = { duplicate: false, unmatched: true, new: true }', javascript)
        self.assertIn('if (transaction._isDuplicate) return "duplicate"', javascript)
        self.assertIn('if (transactionUi.isInternalTransfer(transaction)) return "new"', javascript)
        self.assertIn('if (transaction._classificationMatched === false) return "unmatched"', javascript)
        self.assertIn('state.reviewFilters[importReviewType(state.importedTransactions[index])]', javascript)
        self.assertIn("importedTransactionMatchesFieldFilters", javascript)
        self.assertIn('No transactions match the enabled filters.', javascript)
        ...
        self.assertIn('!transactionUi.isInternalTransfer(transaction) && transaction._classificationMatched === false', javascript)
        self.assertIn('classificationBadge.textContent = "No rule matched"', shared_javascript)
        self.assertIn("transaction-row--needs-classification", shared_javascript)
        self.assertIn("classification-needed-badge", css)
