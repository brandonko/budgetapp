...
def decorate(existing, preview, credits, selections):
    ...
    buckets = purchase_index(existing) if credits else {}
    updated, choices = apply(existing, credits, selections, included, buckets)
    ...
    for staged_id in choices:
        ...
        if (row is None or row["_isDuplicate"] or identity(row) != identity(credits[staged_id])
                or row.get("repaymentTo") or row.get("_linkRole") or row.get("id") in consumed
    ...
    for row in preview:
        ...
        if (credit is None or row["_isDuplicate"] or row.get("_isInternalTransfer")
            ...
            continue
        row["_refundCandidates"] = [candidate for candidate in candidates(existing, row, buckets)
