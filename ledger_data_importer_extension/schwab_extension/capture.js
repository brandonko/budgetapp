...
(() => {
  ...
  window.addEventListener("message", (event) => {
    if (event.source !== window || event.origin !== location.origin || event.data?.source !== "ledger-schwab-control") return;
  ...
  async function emit(candidate, nonce = activeNonce) {
    if (!nonce) return;
    try {
      if (candidate?.size > MAX_BYTES) throw new Error("Schwab CSV exceeds 16 MB.");
      ...
      if (!/withdrawal/i.test(raw.slice(0, 4096)) || !/deposit/i.test(raw.slice(0, 4096))) return;
      if (typeof raw !== "string" || new TextEncoder().encode(raw).length > MAX_BYTES) return;
      if (activeNonce === nonce) window.postMessage({ source: "ledger-schwab-csv", nonce, content: raw }, location.origin);
    } catch {
      // Do not interrupt the bank's download. Ledger's timeout explains a missed capture.
    }
  }
